...
function preload() {
    img=loadImage("https://th.bing.com/th/id/OIP.miT46AxSr4KzTzc3azUYewHaHa?pid=ImgDet&rs=1");
}
...
function draw() {
    ...
    if(statusa!=""){
        for(i=0;i<ans.length; i++){
            ...
            text(ans[i].label+ " "+percent+"% ", ans[i].x+(ans[i].width/2/2),ans[i].y);
            noFill();
            stroke(255,255,255);
            //console.log(ans[i].x/2);
            rect(ans[i].x+(ans[i].width/2/2),ans[i].y,ans[i].width,ans[i].height/2);
